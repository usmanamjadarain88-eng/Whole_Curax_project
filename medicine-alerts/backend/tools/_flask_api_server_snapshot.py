...
@app.route("/user/account-status", methods=["GET"])
def user_account_status():
    ...
    return jsonify({"account_status": st})


@app.route("/maintenance/cleanup-pending", methods=["POST"])
def maintenance_cleanup_pending():
    """POST optional JSON { hours_sessions, hours_users } - requires X-Maintenance-Key matching MAINTENANCE_API_KEY."""
    key = (request.headers.get("X-Maintenance-Key") or "").strip()
...
@app.route("/maintenance/run-alert-checks", methods=["POST"])
def maintenance_run_alert_checks():
