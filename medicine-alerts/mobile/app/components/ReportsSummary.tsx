...
interface ReportsSummaryProps {
  logs: AlertLog[];
  /** Show "User" column in By medicine table (for main admin dashboard); default false */
  showUserColumn?: boolean;
}

export function ReportsSummary({ logs, showUserColumn = false }: ReportsSummaryProps) {
  ...
  const { filtered, taken, missed, skipped, total, adherencePct, byMedicine } = useMemo(() => {
    ...
    const byMedicine: { name: string; userName?: string; taken: number; missed: number; skipped: number; total: number }[] = [];
    const map = new Map<string, { taken: number; missed: number; skipped: number }>();
    filtered.forEach((l) => {
      const key = showUserColumn ? `${l.userName ?? '—'}\t${l.medicineName || 'Unknown'}` : (l.medicineName || 'Unknown');
    ...
    map.forEach((v, key) => {
      ...
      if (showUserColumn) {
        const i = key.indexOf('\t');
        userName = i >= 0 ? key.slice(0, i) : undefined;
        name = i >= 0 ? key.slice(i + 1) : key;
      } else {
        name = key;
      }
      byMedicine.push({
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator style={styles.tableScroll}>
            <View style={styles.table}>
              <View style={[styles.row, styles.headerRow]}>
                {showUserColumn && (
                  <Text style={[styles.cell, styles.headerCell, styles.colUser]}>User</Text>
                )}
              ...
              {byMedicine.map((m) => (
                <View key={showUserColumn ? `${m.userName ?? ''}-${m.name}` : m.name} style={styles.row}>
                  {showUserColumn && (
                    <Text style={[styles.cell, styles.colUser]} numberOfLines={1}>{m.userName ?? '—'}</Text>
                  )}
