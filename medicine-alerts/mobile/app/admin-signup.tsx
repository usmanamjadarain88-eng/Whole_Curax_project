...
import { generateDeviceApiKey, generateDeviceId, saveCredentials } from '../lib/api';
...
export default function AdminSignUpScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (!code) {
      Alert.alert('Validation', 'Please enter Admin Access Code.');
      return;
    }
    const botId = generateDeviceId('admin');
    ...
    const resp = await saveCredentials({
      ...
      access_code: code,
      email: e,
      name: e.split('@')[0] || 'Admin',
      fcm_token: fcmToken,
    });
    if (!resp || (resp.message as string) !== 'ok') {
      Alert.alert('Access Denied', 'Invalid Admin Access Code or backend unavailable.');
      return;
    }
    const accessCode = ((resp.admin_access_code as string) || code || '').trim();
    const connectionCode = ((resp.connection_code as string) || '').trim();
    await setStoredUser({
      email: e,
      password: p,
      role: 'admin',
