    ...
    try:
        ...
        if not response:
            ...
            try:
                ...
                _disconnect_on_serial_error(controller, ex)
                return False, "Device disconnected or not responding"
        up = response.upper()
        ...
        _disconnect_on_serial_error(controller, e)
        return False, ("Device disconnected or not responding" if _is_serial_error(e) else str(e)[:40])
    finally:
        controller.serial_pause = False


def _is_serial_error(e):
    """True if exception is serial-related (timeout, disconnect, etc.)."""
    if e is None:
        return False
    try:
        import serial
        return isinstance(e, (serial.SerialException, serial.SerialTimeoutException, OSError, IOError))
    except Exception:
        return "timeout" in str(e).lower() or "serial" in str(e).lower() or "write" in str(e).lower()


def _disconnect_on_serial_error(controller, e):
    """On write timeout or serial error, disconnect so UI shows Disconnected."""
    if not _is_serial_error(e):
        return
    try:
        from connection import serial_connection
        serial_connection.disconnect_esp32(controller)
    except Exception:
        pass
