    ...
    def apply_theme(self, theme_name: str):
        ...
        try:
            self._title_label.setStyleSheet(
                f"font-size: 14pt; font-weight: 800; color: {self._title_color}; "
                "font-family: 'Segoe UI'; letter-spacing: -0.3px; background: transparent;"
            )
            self._subtitle_label.setStyleSheet(
                f"font-size: 9pt; color: {self._secondary_text_color}; background: transparent;"
            )
            if hasattr(self, "_title_history"):
                self._title_history.setStyleSheet(
                    f"font-size: 14pt; font-weight: 800; color: {self._title_color}; background: transparent;"
                )
        except Exception:
            pass
