    ...
    def apply_theme(self, theme_name: str):
        ...
        for lbl in self.findChildren(QLabel):
            ...
            if any(c in txt for c in ["💊","📦","📅","🕒","📥","⏰","📝"]):
                lbl.setStyleSheet(
                    f"font-size: 8pt; font-weight: 600; color: {label_c}; "
            ...
            try:
                if isinstance(w, QComboBox):
                    w.setStyleSheet(
                        ...
                        f"  border: 1.5px solid {input_bdr}; border-radius: 8px;"
                        f"  background-color: {input_bg}; color: {input_fg};"
                        f"  padding: 7px 10px; font-size: 9pt; min-height: 28px;"
                        f"}}"
        ...
        try:
            self.save_btn.setStyleSheet(
                ...
                f"  font-size: 9pt; padding: 9px 24px; border-radius: 8px; border: none;"
                f"}}"
